<script lang="ts">
	...
	onMount(async () => {
		try {
			const [folderList, config] = await Promise.all([
				getFolders(vault),
				getConfig()
			]);
			folders = folderList;
			const info = config.vaults.find((v) => v.id === vault);
			if (info) vaultName = info.name;
		} catch (e) {
...
<div class="home">
	<h2 class="page-title">{vaultName}</h2>
	...
	{:else}
		<div class="folder-grid">
			{#each folders as folder (folder.path)}
				<a class="folder-card" href="/{encodeURIComponent(vault)}/{encodeURIComponent(folder.name)}">
