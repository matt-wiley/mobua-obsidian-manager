# Emergent schema
_updated: 2026-07-13 | relates: [[markdown-source-of-truth]], [[frontmatter-type-inference]], [[all-sql-in-queries]], [[canonical-field-options]]_
...
This is the direct consequence of [[markdown-source-of-truth]]: the schema can't be authoritative in the DB when the DB is rebuildable. All the SQL that reads these JSON columns is centralized per [[all-sql-in-queries]]. The one deliberate exception is [[canonical-field-options]]: a field can opt into a declared option list (stored in the vault, merged into this endpoint's output) so retired values stay selectable.
